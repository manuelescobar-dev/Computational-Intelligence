...
class Policy(torch.nn.Module):
    def __init__(self, state_space, action_space):
        ...
        # Parameters
        self.first_layer = 128
        self.second_layer = 128
        self.third_layer = 128

        # Layers
        self.f1 = torch.nn.Linear(self.state_space, self.first_layer)
        self.f2 = torch.nn.Linear(self.first_layer, self.second_layer)
        self.f3 = torch.nn.Linear(self.second_layer, self.third_layer)
        self.f4 = torch.nn.Linear(self.third_layer, self.action_space)
        self.init_weights()
...
class Agent(object):
    ...
    def store_outcome(self, observation, action_output, action_taken, reward):
        """
        This function stores the observation, the action taken, and the reward.
        """
