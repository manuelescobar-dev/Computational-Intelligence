...
def train(train_episodes, silent=True, max_timesteps=1000):
    ...
    for episode_number in tqdm(range(train_episodes)):
        ...
        while winner < 0 and timesteps < max_timesteps:
            ...
            if (a and winner == env.current_player_idx) or (
                not a and winner != env.current_player_idx
            ):
                agent.store_reward(1, "A")
                agent.store_reward(-1, "B")
            elif (a and winner != env.current_player_idx) or (
                not a and winner == env.current_player_idx
            ):
                agent.store_reward(1, "B")
                agent.store_reward(-1, "A")
        ...
        agent.episode_finished(episode_number * 2, a_actions, a_rewards)
        agent.episode_finished(episode_number * 2 + 1, b_actions, b_rewards)
