...
class MinimaxPlayer(Player):
    def __init__(self, filename="minimax.npy", depth=5) -> None:
        """# CACHE
        self.filename = filename
        # Load cache
        if os.path.exists(filename):
            self.cache = np.load(filename, allow_pickle=True)
        # Initialize cache
        else:
            # Fill cache with None
            self.cache = np.full((3 ^ 25, 2), None)
            np.save(filename, self.cache)"""
        ...
        """ for i in [0, 4]:
            for j in range(5):
                for move in moves:
                    if not (
                        (i == 0 and move == Move.TOP)
                        or (i == 4 and move == Move.BOTTOM)
                        or (j == 0 and move == Move.LEFT)
                        or (j == 4 and move == Move.RIGHT)
                    ):
                        actions.append(((i, j), move)) """
    ...
    def make_move(self, game: "Game") -> tuple[tuple[int, int], Move]:
        ...
        """ # Look in cache
        if self.cache[self.hash_state(board), 0] is not None:
            return self.cache[self.hash_state(board)]
        # Cache miss
        else:
            player = game.get_current_player()
            player = 1 - player
            # If X is the current player (max)
            action = self.minimax(player, game)

            self.cache[self.hash_state(board)] = action
