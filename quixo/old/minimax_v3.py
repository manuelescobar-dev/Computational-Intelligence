...
class MinimaxPlayer(Player):
    def __init__(self, filename="minimax.npy", depth=4) -> None:
        self.depth = depth
        ...
        # Initialize cache
            ...
            self.cache = np.full((3**25, 2, 8), None)
    ...
    def make_move(self, game: "Game") -> tuple[tuple[int, int], Move]:
        ...
        hash_board = self.hash_state(board)

        # Look in cache
        for d in range(self.depth):
            if self.cache[hash_board, player, d] is not None:
                return self.cache[hash_board, player, d]
        # Cache miss
        action = self.minimax(player, board)
        self.cache[hash_board, player, self.depth] = action
